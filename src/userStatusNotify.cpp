...
UserStatusNotify::UserStatusNotify() {
	...
	_ChatMembers = std::make_shared<ChatMembers>();
	_UserActives = std::make_shared<UserActives>();
}

uint64_t UserStatusNotify::getUID(boost::asio::ip::tcp::socket& client_socket) {
...
void UserStatusNotify::notifyOffline(boost::asio::ip::tcp::socket& client_socket) {
	try {
...
void UserStatusNotify::notifyOffline(uint64_t& UID) {
	try {
...
void UserStatusNotify::notifyOnline(boost::asio::ip::tcp::socket& client_socket) {
	try {
		uint64_t UID = getUID(client_socket);
...
void UserStatusNotify::notifyOnline(uint64_t& UID) {
	try {
		std::vector <uint64_t> interlocutors = _ChatMembers->getListOfInterlocutors(UID);
